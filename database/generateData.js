...
var dlcId = 1;
const allUserTags = userTagsStr.split(',').join();
...
const generateDlc = function (associatedGameId, n) {
  ...
  for (var i = 0; i < n; i++) {
    ...
    while (numTags) {
      let idx = Math.floor(Math.random() * allUserTags.length);
      let tag = userTags[idx];
    ...
    while (numPreviews) {
      ...
      let url = faker.image.imageUrl(600, 338, 'food', true);
...
const init = function () {
  ...
  for (var i = 0; i < 100; i++) {
    let numDlc = Math.floor(maxDlc * Math.random());
